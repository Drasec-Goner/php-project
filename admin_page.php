<?php
	...
	if(isset($_POST['upload_bengali'])){
		...
		if(isset($_FILES['audio_file'])){
			...
			move_uploaded_file($file_tmp,"songs/bengali_songs/".$file_name);
         }
         ...
         if(isset($_FILES['song_image'])){
			...
			move_uploaded_file($file_tmp,"songs/bengali_songs/img/".$file_name);
		 }

		$sql = "INSERT INTO bengali_albums(`song_name`,`song_format`,`singer_name`,`movie_name`,`song_image`,`audio_file`) VALUES('$song_name','$song_format','$singer_name','$movie_name','$song_image','$audio_file')";
		$result = mysqli_query($conn,$sql);
		if($result){
    ...
    $sql = "SELECT * FROM bengali_albums ORDER BY song_id ASC";
	$result = mysqli_query($conn,$sql);
	...
	while($rows = mysqli_fetch_array($result)){
		...
		if(isset($_POST[$song_id])){
			...
			$sql = "SELECT * FROM bengali_albums WHERE song_id = '$song_id' ";
			...
			$sql = "DELETE FROM bengali_albums WHERE song_name = '$song_name' ";
			$results = mysqli_query($conn,$sql);

			if($results){
				echo '<script type="text/javascript">';
                echo 'setTimeout(function () { sweetAlert("Deleted"," <b>Song '.$song_name.' is deleted from bengali albums</b>","success");';
				echo '}, 500);</script>';
			}else{
				echo '<script type="text/javascript">';
...
?>
    ...
    <div class="alert alert-primary" role="alert">
         <center><b>Bengali Songs</b></center>
    </div>
	...
	<div class="form-row">
		...
		<div class="col">
			<div class="text-center">
				<button type="submit" name="upload_bengali" class="btn btn-default btn-rounded mb-4">Upload <i class="fa fa-upload"></i></button>
			</div>
		</div>
	</div>	
</form>
...
<?php 
    ...
    $sql = "SELECT * FROM bengali_albums order by song_id";
    $result = mysqli_query($conn,$sql);
